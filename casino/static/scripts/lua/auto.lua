...
-- ============================================================================
-- 📋 СПИСОК ИСКЛЮЧЕНИЙ "ИМЯ ШМОТКИ → РЕАЛЬНЫЙ ID В ИНВЕНТАРЕ БОТА"
-- ============================================================================
-- Зачем: в Roblox MM2 у некоторых годли название одно, а внутренний ID совсем
-- другой (legacy-ID, переименования и т.д.). Например, "Cookiecane" по факту
-- лежит в инвентаре под ID "Gingermint_K". Из-за этого автомат не находит
-- шмотку и не может её вывести.
--
-- ✏️ КАК ДОБАВЛЯТЬ СВОИ ИСКЛЮЧЕНИЯ:
--   1) Слева — имя шмотки В НИЖНЕМ РЕГИСТРЕ, ровно как приходит с сайта
--      (с пробелами, например: "elderwood revolver", "cookiecane").
--   2) Справа — точный ID, как лежит в инвентаре бота (см. лог
--      "=== ИНВЕНТАРЬ БОТА ===" — там как раз отображаются ID).
--   3) Для Chroma-варианта МОЖНО (но не обязательно) добавить отдельную
--      запись с префиксом "chroma ". Если её нет, скрипт сам попробует
--      <BaseID>Chroma и Chroma<BaseID> на основе базового исключения.
--
-- Примеры:
...
-- ============================================================================
local ID_EXCEPTIONS = {
    -- Известные пушки с нестандартным/legacy ID:
    ["elderwood revolver"]   = "ElderwoodGun",
    ["cookiecane"]           = "Gingermint_K",
    ["chroma cookiecane"]    = "Gingermint_KChroma",

    -- ⬇️ ДОБАВЛЯЙ СВОИ ИСКЛЮЧЕНИЯ НИЖЕ ⬇️
    -- ["имя_с_сайта_в_нижнем_регистре"] = "ТочныйIDИзИнвентаря",
}
...
local function FindItemIDInInventory(itemLabel, inventory)
    ...
    if isChroma then
        ...
        -- Деривация из ID_EXCEPTIONS для базы (например, "cookiecane" → "Gingermint_K"):
        -- если в исключениях есть только база, без отдельной chroma-записи —
        -- автоматически добавляем "<BaseID>Chroma" и "Chroma<BaseID>" в варианты.
        local baseExceptionID = ID_EXCEPTIONS[string.lower(baseName)]
        if baseExceptionID then
            print("     📋 Деривация по ID_EXCEPTIONS: base='" .. baseName .. "' → '" .. baseExceptionID .. "'")
            table.insert(variants, baseExceptionID .. "Chroma")
            table.insert(variants, "Chroma" .. baseExceptionID)
            table.insert(variants, baseExceptionID)
        end
